...
$\sum_{i=1}^\infty w'(i)  \lambda^i = \sum_{i=1}^\infty w(i)\lambda^i - \sum_{i=1}^\infty a  \lambda^i = \sum_{i=1}^\infty w(i) \lambda^i - \frac{a}{1-\lambda}$.
...
$\sum_{i=1}^\infty w''(i)  \lambda^i = \frac{1}{b'} \sum_{i=1}^\infty w'(i)  \lambda^i$.
%$w''=w'/b'$. 
...
We begin with establishing a normal form of \GTDS, in which the weights are natural numbers
and the least weight is $0$. Such a normal form generalizes \TDSzo.
...
\begin{exampleE}
Consider a \GTDS with $\lambda = \frac{3}{5}$, weights $\frac{-1}{2}, 0, \frac{1}{2},\frac{2}{3}$ and the target $\frac{-99}{500}$. It reduces to 
a \GTDS instance with the same $\lambda = \frac{3}{5}$, natural weights $0,3,6,7$ and the target $\frac{414}{125}$.
\end{exampleE}
...
\todo{multiply the target by $\lambda$}
...
Moreover, if there is an eventually-periodic solution to $\P$, then there is also a solution $u v^{\omega}$ with $|u| + |v| \leq 4\cdot d \cdot |e| \cdot \Dm{k}$.
\end{restatable}
\begin{proof}
In the case of $\GTDS$, the decision procedure is analogous to the proof of Lemma~\ref{lem:EP}, with the following differences: i) Rather than considering the presentation generated by the greedy exploration, we consider some representation generated by the nondeterministic exploration; ii) The upper bound on relevant gaps is $\frac{\lambda \Dm{k}}{1 - \lambda}\leq \Dm{k}$; 
iii) The first gap is $t \cdot \lambda$ rather than $t$, i.e., we multiply the target by $\lambda$ 
to make it consistent with \TDSzo, where the summation starts with $i=1$, 
and iv) A decision is nondeterministically reached within $d \cdot \Dm{k}$ steps.
...
\begin{exampleE}
Consider the \CGTDS instance with a discount factor $\lambda=\frac{2}{3}$, the target $t=\frac{12}{5}$,
and weights $a=0$, $b=1$, and $c=2$.
Furthermore, we consider an $\omega$-regular constraint 
$e := \Sigma^*(\Sigma a)^\omega$, namely eventually the letter a occurs at every second position.

We follow the proof of Theorem~\ref{thm:GTDSEventuallyPeriodic}. The first gap is $t \cdot \lambda = \frac{8}{5}$.
The minimal recoverable gap is $0$, and the maximal recoverable gap is 
$\frac{\lambda c}{1-\lambda} = \frac{\frac{2}{3} 2}{\frac{1}{3}} = 4$. 

Now, we build the safety automaton $\A$ of the relevant gaps along the exploration of $t$. 
First, we build a labelled graph of gaps corresponding to all explorations starting from $\frac{8}{5}$ 
and prune it, once 
it is clear that the exploration does not lead to an ultimately periodic representation. 
Therefore, 
We only consider recoverable gaps whose denominator is 5, i.e., between $\frac{0}{5}$ and $\frac{20}{5}$. 
Moreover, following the argument in the proof of Lemma~\ref{lem:EP}, 
we can omit further exploration of gaps whose numerator is odd 
as all its continuations will have denominators growing to infinity. Thus, there can be no ultimately-periodic  
representation. 
\begin{figure}
\begin{tikzpicture}
\tikzset{good/.style={black, draw=blue,thick, fill=green!20,minimum size=1.1cm}}
\tikzset{tooBig/.style={black, draw=red,thick, fill=red!20, minimum size=1.1cm}}
\tikzset{odd/.style={black, draw=yellow,thick, fill=yellow!20, minimum size=1.1cm}}

\foreach \n/\x/\y\l/\f in {Eps/0/0/{\frac{8}{5}}/good,
...
\node[draw,circle,good] (q0) at (0,0) {$\frac{8}{5}$};
\node[draw,circle,good] (q1) at (0,-3) {$\frac{12}{5}$};
...
\caption{The graph of pruned explorations is on the left, while the corresponding B\"uchi automaton $\A$ is on the right. }

\end{figure}

The nondeterministic B\"uchi automaton $\B$ representing the constraint $e$ is presented in the figure below along
with the B\"uchi automaton $\C$ that recognizes the intersection of the languages of $\A$ and $\B$. 
Notice that the intersection of a safety automaton and a B\"uchi automaton is the B\"uchi automaton 
derived by their product, and in taking the product we can ignore dead-end states.
...
\node[draw,circle] (q2) at (4,0) {$q_2$};

\node[draw,circle,minimum size=0.5cm] (q1acc) at (2,0) {};
\node[draw,circle,minimum size=0.5cm] (q2acc) at (4,0) {};
...
\foreach \lab/\gap/\y in {A/{\frac{8}{5}}/0, B/{\frac{12}{5}}/-3}
{
	\node[draw,circle] (q0\lab) at (0,\y) {$q_0,\gap$};
	\node[draw,circle] (q1\lab) at (2.5,\y) {$q_1,\gap$};
	\node[draw,circle] (q2\lab) at (5,\y) {$q_2,\gap$};
}

\draw[->,bend right] (q0A) to node[left]{a} (q0B);
\draw[->,bend right] (q0B) to node[left]{c} (q0A);

\draw[->, bend left=10] (q0B) to node[above, yshift=15pt, xshift=10pt ] {c} (q1A);
\draw[->,bend left=10] (q0A) to node[below, yshift=-15pt,xshift=10pt ]{a} (q1B);

\draw[->,bend right=20] (q1B) to node[below]{c} (q2A);
\draw[->, bend left] (q1A) to node[above]{a} (q2B);

\draw[->,bend right=20] (q2A) to node[above]{a} (q1B);

\end{scope}

\end{tikzpicture}

\caption{The B\"uchi automaton $\B$ for $e$ is on the left, while the product automaton $\C$ is on the right.}

\end{figure}

The language of $\C$ is not empty; it consists of a single ultimately periodic word $w=(ac)^\omega$.
Observe that indeed, $(\Dz.(20)^{\omega})_{[\frac{1}{\lambda}]}$ is the required target $\frac{12}{5}$.
\end{exampleE}

\begin{exampleE}
In contrast to the above example, ultimately-periodic solutions to $\CGTDS$ does not need to be unique.
...
% \begin{exampleE}
% Consider a \CGTDS instance with the discount factor $\lambda = \frac{3}{5}$, 
% natural weights $0,3,6,7$ and the target $\frac{51}{8}$.

% The greedy exploration works as follows.
% The initial gap is $\frac{414}{125}$ and the numerator $3$ of $\lambda$ divides $414$. 
% The maximal possible weight is $3$ and the next gap is $\frac{63}{125}$. 
% Again, $3$ divides $63$ and the maximal weight to keep the gap non-negative is $3$. The resulting gap is $\frac{6}{5}$.
% While $3$ divides $6$ and the only weight to keep the gap non-negative is $0$, but then the gap becomes $2$ and the numerator is not divisible by $3$.

% However, observe that $\frac{414}{125}$ can be represented with the expansion $(\Dz.307)_{[\frac{1}{\lambda}]}$:
% \[
% \frac{414}{125} = 3\cdot \frac{3}{5} + 0 \cdot \left(\frac{3}{5}\right)^2 + 7 \cdot \left(\frac{3}{5}\right)^3 = \frac{225}{125} + \frac{189}{125}
% \]
% the sequence of gaps is as follows: $\frac{414}{125}, \frac{63}{25}, \frac{21}{5}$ and finally $0$.
% \end{exampleE}
...
Consider an instance $\P$ of \CGTDSF (or \GTDSF) with a regular constraint $e$, and a rational discount factor $\lambda=\frac{p}{q}$, a target $t=\frac{c}{d}$, and weights $\Dm{1} < \Dm{2} < \ldots < \Dm{k} \in \Rat$ with common denominator $m\in\Nat$. Then the problem of whether $\P$ has an eventually-periodic solution is decidable in PSPACE.
Moreover, if there is a solution to $\P$, then there is also a solution of length up to $4\cdot d(q-p) \cdot (|e|+2) \cdot 2\max(|\Dm{k}|,|\Dm{1}|) \cdot m$.
...
By the proof of Theorem~\ref{thm:GTdsToGTdszo}, an instance $\P'$ of \CGTDS can be reduced to an instance $\P''$ of \CGTDS in normal form, where the new target is $t''= (t\cdot m) - \frac{\Dm{1}\cdot m\cdot\lambda}{1 - \lambda} = \frac{c\cdot m (q-p) - d(\Dm{1}\cdot m \cdot p)}{d(q-p)}$ and the new highest weight is $\Dm{k}''=(\Dm{k}-\Dm{1})m$ if $\Dm{1}<0$ and $\Dm{k}>0$, or $\Dm{k}''=|\Dm{k}|m$ if $\Dm{1}\geq 0$ and $\Dm{k}>0$, or $\Dm{k}''=|\Dm{1}|m$ if $\Dm{1}<0$ and $\Dm{k}\leq0$, implying that in any case $\Dm{k}''\leq 2\max(|\Dm{k}|,|\Dm{1}|) \cdot m$.

Hence, the PSPACE decidability and the bound on a shortest witness directly follows from Theorem~\ref{thm:GTDSEventuallyPeriodic}.
%See Example E at the end of the section.
\end{proof}
...
% The following example, which does not appear in the LICS paper, has no number for keeping the numbering of the other theorems and alike as in the LICS paper.
%\begin{exampleE}
%\todo{Add a detailed example of resolving \CGTDSF, going through all the steps in the proofs of Theorems~\ref{t:FiniteTdsToCgtds},\ref{thm:GTdsToGTdszo},\ref{thm:GTDSEventuallyPeriodic},\ref{thm:GTDSFinite}.} 
%\end{exampleE}
