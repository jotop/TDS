...
a \GTDS instance with the same $\lambda = \frac{3}{5}$, natural weights $0,3,6,7$ and the target $\frac{789}{125}$.
\end{exampleE}
...
We follow the proof of Theorem~\ref{thm:GTDSEventuallyPeriodic} for cheking whether it has an eventually-periodic solution. 
...
$\frac{\lambda c}{1-\lambda} = \frac{\frac{2}{3} 2}{\frac{1}{3}} = 4$, and all relevant gaps have denominator $5$ (as otherwise they cannot be continued to an eventually-periodic solution). 
...
as all its continuations will have denominators different from $5$. 
\begin{figure}
...
\node[anchor=west]  (A_TooBig) at (-0.75,1.9) {When ignoring dead-end states};
	
\node[state, initial above] (q0) at (0,0) {$\frac{8}{5}$};
\node[state] (q1) at (0,-3) {$\frac{12}{5}$};
...
\caption{The automata of the example for the proof of Theorem~\ref{thm:GTDSEventuallyPeriodic}: The safety automaton $\A$ for the exploration of the relevant gaps (stopping the exploration for too big/small gaps and for gaps with odd numerators), the B\"uchi automton $\B$ expressing the $\omega$-regular constraint, and the B\"uchi automton $\C$ for the intersection of $\A$ and $\B$.}
\label{fig:EP}
...
Moreover, if there is a solution to $\P$, then there is also a solution of length up to $4\cdot d(q-p) \cdot (|e|+2) \cdot \max(|\Dm{k}|,|\Dm{1}|) \cdot \frac{m}{1-\lambda}$.
